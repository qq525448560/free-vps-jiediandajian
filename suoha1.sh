  ...
  while true; do
    ...
    if [ $cloudflared_running -eq 0 ]; then
      ...
      if [ -f "$SUOHA_DIR/cloudflared" ] && [ -f "$SUOHA_DIR/xray/config.json" ]; then
        ...
        # 从文件读取IP版本，如果文件不存在则使用默认值4
        if [ -f "$SUOHA_DIR/ip_version" ]; then
          saved_ips=$(cat "$SUOHA_DIR/ip_version")
        else
          saved_ips="4"
        fi
        "$SUOHA_DIR/cloudflared" tunnel --url "http://localhost:$port" --no-autoupdate --edge-ip-version "$saved_ips" --protocol http2 > "$SUOHA_DIR/argo.log" 2>&1 &
